...
def extract_course_times(course=COURSE):
    ...
    """
    pattern = r'\d{2}:\d{2}'
    timings = re.findall(pattern, course)
    return timings
...
def get_all_hashtags_and_links(tweet=TWEET):
    """Get all hashtags and links from the tweet text
       ...
       Return this list.
    """
    pattern = r'(http[s]?://\S+|#\w+)'
    hashtags_and_links = re.findall(pattern, tweet)
...
def match_first_paragraph(html=HTML):
    ...
    """
    pattern = r'<p>(.*?)</p>'
    html_content = re.search(pattern, html)
    if html_content:
        return html_content.group(1)
    return None


print(extract_course_times(course=COURSE))
print(get_all_hashtags_and_links(tweet=TWEET))
print(match_first_paragraph(html=HTML))